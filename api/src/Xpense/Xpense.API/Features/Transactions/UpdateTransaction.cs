...
public sealed class UpdateTransaction : IEndpoint
{
    ...
    public sealed class Validator : AbstractValidator<Request>
    {
        public Validator()
        {
            ...
            RuleFor(request => request)
                ...
                .WithMessage("Either a source account or a destination account is required.")
    ...
    public static void Map(IEndpointRouteBuilder app) =>
        app.MapPut("/api/v1/transactions/{id:int}", Handle).WithName(nameof(UpdateTransaction)).Validated().BlocksDuringLegacyClaim();

    private static async Task<Ok<TransactionResponse>> Handle(
        int id,
        Request request,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var occurredAt = request.OccurredAt?.UtcDateTime ?? transaction.OccurredAt;
        ...
        var replacement = source is not null && destination is not null
            ? Transaction.Transfer(source, destination, amount, request.Reason, resolvedTags, occurredAt)
            : await OneSided(
                dbContext,
                merchants,
                request,
        ...
        transaction.ReplaceWith(replacement);

        if (await dbContext.SaveChangesAsync(cancellationToken) < 1)
            throw new TransactionUpdateFailedException(id);
        ...
        CancellationToken cancellationToken)
    {
        ...
        return destination is not null
            ? Transaction.Income(destination, amount, category, merchant, tags, occurredAt)
            : Transaction.Expense(source!, amount, category, merchant, tags, occurredAt);
    }
